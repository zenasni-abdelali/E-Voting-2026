const API_BASE_URL = " https://4db5-154-121-42-196.ngrok-free.app";
...
function persistAdminIdentity(userLike = {}, fallbackEmail = '') {
    const email = normalizeEmail(userLike.email || userLike.Email || fallbackEmail);
    const nom = String(userLike.nom || userLike.Nom || '').trim();
    localStorage.setItem('evote_user', JSON.stringify({ role: 'admin', nom, email }));
    if (email) {
...
/** Déverrouille la clé admin et la stocke en session (requis pour créer une élection / dépouiller). */
async function hydrateAdminSigningSession(email, password) {
    const res1 = await fetch(`${API_BASE_URL}/api/admin/auth-step1`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
    });
    const data1 = await res1.json();
    if (!res1.ok || data1.status !== 'existe') return { ok: false };
    if (typeof data1.enc_k !== 'string' || !data1.enc_k.trim().startsWith('{')) return { ok: false };

    const privateKey = await unwrapP256PrivateKey(password, data1.sel, data1.iv, data1.enc_k);
    const signature = await signChallenge(privateKey, data1.challenge);
    const res2 = await fetch(`${API_BASE_URL}/api/admin/auth-step2`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, signature }),
    });
    const data2 = await res2.json();
    if (!res2.ok || data2.status !== 'valide') return { ok: false };

    const pkcs8 = await crypto.subtle.exportKey('pkcs8', privateKey);
    sessionStorage.setItem('admin_private_pkcs8_b64', u8ToB64(new Uint8Array(pkcs8)));
    sessionStorage.setItem('admin_email', email);
    return { ok: true, user: data2 };
}

async function startInscription() {
    const nom = document.getElementById('admin-fullname').value;
    const email = normalizeEmail(document.getElementById('admin-email').value);
    const password = document.getElementById('admin-mdp').value;
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/admin/inscription`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function verifyOTP() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (password) {
                try {
                    const sessionResult = await hydrateAdminSigningSession(email, password);
                    if (!sessionResult?.ok) {
                        showMessage('Compte créé. Reconnectez-vous pour activer la signature des élections.', 'error');
                    }
                } catch (_) {
                    showMessage('Compte créé. Reconnectez-vous pour activer la signature des élections.', 'error');
                }
            ...
            persistAdminIdentity(data, email);
            window.location.href = 'Espace-administrateur.html';
        } else showMessage('Erreur: ' + (data.detail || 'Code OTP invalide.'), 'error');
    } catch (error) {
        console.error(error);
        showMessage('Impossible de verifier le code.', 'error');
    }
}
...
async function loginAdmin() {
    ...
    try {
        ...
        if (sessionResult?.ok) {
            ...
            persistAdminIdentity(sessionResult.user || { email }, email);
            window.location.href = 'Espace-administrateur.html';
            return;
        }
        showMessage('Accès refusé : identifiants invalides ou compte non compatible P-256.', 'error');
    } catch (error) {
        console.error(error);
        showMessage("Impossible de terminer l'authentification.", 'error');
    }
}
