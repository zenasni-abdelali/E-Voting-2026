...
/* ─── Config ─────────────────────────────────────── */
const API_BASE_URL = " https://ffa7-154-121-40-230.ngrok-free.app";
...
async function openElectionResultsView(electionId) {
    ...
    try {
        ...
        // ─── CORRECTIF : Déclaration de 'mode' AVANT son utilisation ───
        const mode = (election?.affichage_resultats || resultsData?.affichage_resultats || "").toLowerCase();

        // Évaluation sécurisée pour autoriser l'audit en mode complet ou partielle
        const showGlobalAudit = mode === "complet" || mode === "partielle";
        ...
        console.error("DEBUG - Failed inside openElectionResultsView:", err);

        openActionView(
            "Résultats d'élection",
            `<div class="p-4 bg-red-50 border border-red-200 rounded-xl space-y-2">
                <p class="text-sm font-semibold text-red-700">Erreur de traitement détectée :</p>
                <code class="block text-xs font-mono text-red-600 bg-white p-2 rounded border border-red-100 break-all">
                    ${escapeHtml(err.message || err)}
                </code>
                <p class="text-[11px] text-slate-500">Consultez votre terminal FastAPI backend ou la console développeur (F12) pour voir la trace complète.</p>
            </div>`
        );
    }
}
...
async function runDepouillementFlow(electionId) {
    ...
    // Initialisation des balises temporelles globales
    const globalStart = performance.now();
    console.log(`%c[AUDIT CRYTPO] 🚀 Lancement du flux de dépouillement complet pour l'élection #${electionId}`, "color: #4f46e5; font-weight: bold; font-size: 13px;");

    try {
        const formattedId = isNaN(electionId) ? electionId : parseInt(electionId, 10);

        // ════════ ÉTAPE 1 : INITIALISATION & EXTRACTION DES PACKETS ════════
        const step1Start = performance.now();
        console.log("%c[Étape 1/5] 🔍 Requête /depouillement/start (Vérification et récupération des clés chiffrées)...", "color: #d97706;");
        ...
        if (!r1.ok) {
            ...
            console.error(`❌ Échec à l'Étape 1 [${(step1End - step1Start).toFixed(0)}ms] :`, errMsg);
            showToast(errMsg, "error");
            return;
        }
        console.log(`%c  -> Étape 1 complétée en ${(step1End - step1Start).toFixed(0)} ms.`, "color: #059669;");

        // ════════ ÉTAPE 2 : DÉCHIFFREMENT LOCAL DE LA CLÉ D'ÉLECTION ════════
        const step2Start = performance.now();
        console.log("%c[Étape 2/5] 🔑 Déchiffrement local de la clé de l'élection ", "color: #d97706;");
        ...
        if (!/^[0-9a-fA-F]{64}$/.test(kElectionHex)) {
            console.error(`❌ Échec à l'Étape 2 [${(step2End - step2Start).toFixed(0)}ms] : Clé d'élection invalide.`);
            showToast(tAdmin("depInvalidKey"), "error");
            return;
        }
        console.log(`%c  -> Étape 2 complétée en ${(step2End - step2Start).toFixed(0)} ms. (Clé d'élection extraite localement)`, "color: #059669;");

        // Enveloppement de la clé pour le serveur de calcul
        const wrapped = await wrapElectionKeyForDepServer(j1.server_pubkey_hex, kElectionHex.toLowerCase());

        // ════════ ÉTAPE 3 : DÉCHIFFREMENT DU SCRUTIN & COMPTAGE SERVEUR ════════
        const step3Start = performance.now();
        console.log("%c[Étape 3/5] 🧮 Requête /tally-session (Déchiffrement asymétrique en masse des bulletins et agrégation des résultats)...", "color: #d97706;");
        ...
        if (!r2.ok) {
            console.error(`❌ Échec à l'Étape 3 [${(step3End - step3Start).toFixed(0)}ms] :`, j2.detail || j2.message);
            showToast(j2.detail || j2.message || "Échec du décompte côté serveur.", "error");
            return;
        }
        console.log(`%c  -> Étape 3 complétée en ${(step3End - step3Start).toFixed(0)} ms. (Résultats calculés en mémoire serveur)`, "color: #059669;");
        ...
        // ════════ ÉTAPE 4 : SIGNATURE CRYPTOGRAPHIQUE LOCALE ════════
        const step4Start = performance.now();
        console.log("%c[Étape 4/5] ✍️ Calcul local de la signature P-256 sur le manifeste de clôture...", "color: #d97706;");

        const messageToSign =
            j2.global_message ||
            (await sha256Utf8Hex(`${j2.canonical_for_signing || ""}${j2.hash_liste_bulletins || ""}`));
        if (!messageToSign || messageToSign.length !== 64) {
            console.error("❌ Échec à l'Étape 4 : Message de signature global invalide.");
        ...
        console.log(`%c  -> Étape 4 complétée en ${(step4End - step4Start).toFixed(0)} ms. (Manifeste signé de manière intègre)`, "color: #059669;");

        // ════════ ÉTAPE 5 : PUBLICATION OFFICIELLE DU SCELLÉ ════════
        const step5Start = performance.now();
        console.log("%c[Étape 5/5] 🌐 Envoi de la signature au serveur /publish (Écriture finale et gel en base de données)...", "color: #d97706;");
        ...
        if (!r3.ok) {
            console.error(`❌ Échec à l'Étape 5 [${(step5End - step5Start).toFixed(0)}ms] :`, j3.detail || j3.message);
            showToast(j3.detail || j3.message || "Signature refusée ou publication impossible.", "error");
            return;
        }
        console.log(`%c  -> Étape 5 complétée en ${(step5End - step5Start).toFixed(0)} ms. (Données figées publiquement)`, "color: #059669;");

        // Fin du chronomètre global
        const globalEnd = performance.now();
        const totalDurationMs = (globalEnd - globalStart).toFixed(0);

        // ════════ CONSOLE PERFORMANCE AUDIT REPORT ════════
        console.log(
            `\n%c📊 RAPPORT FINALE D'AUDIT TECHNIQUE — FLUX DE DÉPOUILLEMENT\n` +
            `==================================================================\n` +
            `⏱️ Étape 1 (Serveur - Start Protocol)          : ${(step1End - step1Start).toFixed(0)} ms\n` +
            `⏱️ Étape 2 (Client - Déchiffrement local XOR)  : ${(step2End - step2Start).toFixed(0)} ms\n` +
            `⏱️ Étape 3 (Serveur - Tally & Decrypt votes)  : ${(step3End - step3Start).toFixed(0)} ms\n` +
            `⏱️ Étape 4 (Client - Signature ECDSA P-256)    : ${(step4End - step4Start).toFixed(0)} ms\n` +
            `⏱️ Étape 5 (Serveur - Publication & BDD)       : ${(step5End - step5Start).toFixed(0)} ms\n` +
            `------------------------------------------------------------------\n` +
            `🚀 DURÉE TOTALE DE L'OPÉRATION BIOMÉTRIQUE    : ${totalDurationMs} ms (${(totalDurationMs / 1000).toFixed(2)} secondes)\n` +
            `==================================================================`,
            "color: #1e1b4b; background-color: #e0e7ff; font-family: monospace; font-size: 11px; padding: 8px; border-radius: 6px; font-weight: bold;"
        );

        showToast(j3.message || "Résultats publiés.", "success");
        await loadDashboardData();
    } catch (e) {
        console.error("%c❌ Erreur système critique durant l'exécution :", "color: #b91c1c; font-weight: bold;", e);
