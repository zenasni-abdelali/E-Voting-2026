...
/* ─── Config ─────────────────────────────────────── */
const API_BASE_URL = "https://d55c-41-97-173-116.ngrok-free.app";
        ...
        <div class="space-y-3">
            <details class="group border border-slate-200 rounded-xl bg-white overflow-hidden shadow-sm mb-3">
                ...
                <div class="p-4 border-t border-slate-100 bg-white text-xs text-slate-600 space-y-3">
                    <div>
                        <strong class="text-slate-700 block mb-1">Clé d'empreinte de la liste des votes :</strong>
                    ...
                    <div>
                        <strong class="text-slate-700 block mb-1">Badge d'authentification de l'administrateur :</strong>
                    ...
                    <div>
                        <strong class="text-slate-700 block mb-1">Identifiant de sécurité de l'administrateur :</strong>
                    ...
                    <table class="w-full text-left text-xs border-collapse min-w-[640px]">
                        <thead>
                            <tr class="bg-slate-50 text-slate-500 font-bold border-b border-slate-200 text-[10px] uppercase">
                                ...
                                <th class="py-2 px-3">Numéro du reçu de vote</th>
                                <th class="py-2 px-3">Preuve de validation</th>
                                <th class="py-2 px-3">Code anonyme de l'électeur</th>
                                <th class="py-2 px-3">Option ou candidat choisi</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${bulletinsRowsHtml}
...
async function runChoiceVerification(election, choiceData) {
    ...
    try {
        ...
        // REFORMULÉ : Messages de statut grand public
        const iconHtml = proofValid
            ? `<div class="w-20 h-20 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-4" aria-hidden="true">
               ...
               <p class="text-sm text-emerald-700 mt-2 max-w-md mx-auto">Le système confirme que votre bulletin est authentique et bien comptabilisé. L'anonymat visuel de votre choix reste entièrement protégé.</p>`
            : `<div class="w-20 h-20 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-4" aria-hidden="true">
               ...
               <p class="text-sm text-red-700 mt-2 max-w-md mx-auto">L'authenticité de ce reçu de vote ne peut pas être confirmée par le système. Veuillez signaler cette anomalie.</p>`;
        ...
        renderChoiceVerificationPanel(`
            <div class="rounded-2xl border ${statusClass} p-6 text-center">
                ...
                ${iconHtml}
                <p class="text-[11px] text-slate-500 mt-4">Analyses : Signature numérique ${sigValid ? "Valide ✓" : "Invalide ✗"}, Empreinte du vote ${hashValid ? "Correcte ✓" : "Incorrecte ✗"}, Preuve d'anonymat ${zkpValid ? "Confirmée ✓" : "Erreur ✗"}</p>
            </div>
            <div class="flex flex-wrap justify-center gap-2 mt-5">
                ${proofValid ? `<button type="button" id="choice-proof-confirm" class="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold shadow-md active:scale-[0.98] transition-all text-sm">Confirmer le reçu</button>` : ""}
                <button type="button" id="choice-proof-signal" class="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold shadow-md active:scale-[0.98] transition-all text-sm">Signaler une erreur</button>
            </div>
        `);

        document.getElementById("choice-proof-confirm")?.addEventListener("click", () => confirmChoiceVerification(election, computedN, packet));
        document.getElementById("choice-proof-signal")?.addEventListener("click", () => signalChoiceVerification(election, computedN, packet));
    } catch (error) {
        renderChoiceVerificationPanel(`
            <p class="text-xs font-bold uppercase tracking-[0.18em] text-red-700">Contrôle du vote</p>
            <p class="text-sm text-red-700 mt-2">${error.message || "Impossible d'effectuer les contrôles de sécurité pour le moment."}</p>
        `);
    }
}
