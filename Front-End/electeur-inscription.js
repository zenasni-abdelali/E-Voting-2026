const URL = " https://4db5-154-121-42-196.ngrok-free.app";
...
async function loginElector() {
    const email = document.getElementById('elector-email-log').value.trim();
    ...
    try {
        if (typeof window.evoteHydrateWcSigningFromLogin !== 'function') {
            showMessage("Erreur système : Le module de signature cryptographique est introuvable.", 'error');
            return;
        }
        ...
        // Fix: Force exit if the server returns an error code (404, 400, etc.) or explicit status
        if (!res1.ok || data1.status === 'inexistant') {
            showMessage(data1.detail || 'Compte électeur inexistant. Veuillez vous inscrire.', 'error');
            return;
        }

        // Run the WebCrypto decentralized challenge decryption & signature verification
        ...
        // If your script reaches this point, authentication is verified. Safe to store.
        showMessage('Authentification réussie !', 'success');

        localStorage.setItem('evote_user', JSON.stringify({
            role: 'elector',
            nom: data2.nom || 'Électeur',
            email: data2.email || email
        }));

        // Secure delay to allow the success toast to be seen before reloading tracking states
        setTimeout(() => {
            window.location.href = 'Espace-electeur.html';
        }, 800);
        ...
        console.error(error);
        showMessage(error.message || "Identifiants ou clés de chiffrement invalides.", 'error');
    }
}
